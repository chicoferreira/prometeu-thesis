...
By default, this template generates documents in English.

To use the Portuguese version of the template, set the `language` parameter to `"pt"` in the `thesis` function located in your `main.typ` file, as shown below:
...
For translated content such as "Acknowledgements" or "Statement of Integrity", you will need to do the translations yourself, as these are not provided by the template. Official translations can be found in the [official template](https://web.di.uminho.pt/sitedi/latex/).

Note that logos are automatically adjusted when the `language` parameter is set to `"pt"` and provided `school` is written in Portuguese (e.g., `school: [Escola de Engenharia]`).
