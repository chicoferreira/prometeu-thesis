...
It is based on the [official LaTeX template provided by the Department of Informatics](https://web.di.uminho.pt/sitedi/latex/) and follows the [university’s required formatting standards and guidelines](https://alunos.uminho.pt/PT/estudantes/Paginas/InfoUteisFormatacao.aspx). The current version is checked against [Despacho RT-80/2026](https://alunos.uminho.pt/PT/estudantes/Formataes/1_Despacho%20RT-80-2026.pdf). 

Where the university rules are unclear, or conflict with the materials the university publishes, the template chooses to follow the Department's template instead, while providing improvements in features, usability and presentation. It is designed to be as extensible, customizable, and comprehensible as possible right out of the box.
...
```
typst init @preview/prometeu-thesis:1.1.0
```
...
Out of the box, the template's cover fields, section titles, and the university name are in English.

To switch these to Portuguese, set the `language` parameter to `"pt"` in the `#show: thesis` call within your `main.typ` file.

This does **not** automatically translate your thesis body text or the preamble pages' content (e.g., "Acknowledgements" or the "Statement of Integrity""). You will need to write your own translations for those, as they are not included with the template. You can refer to the official LaTeX template for the original Portuguese translations: [official template](https://web.di.uminho.pt/sitedi/latex/).

## Current Limitations

Currently, this template only supports covers for Master's theses and does not yet accommodate Doctoral (PhD) theses.
